...
export class ResendReceivingClient implements ReceivingClient {
  ...
  async retrieve(event: EmailReceivedEvent, providerEventId: string): Promise<InboundEmail | null> {
    const response = await this.resend.emails.receiving.get(event.data.email_id, { html_format: "cid" });
    if (response.error || !response.data) throw new Error("Resend could not retrieve the received email.");
    const email = response.data;
    if (email.id !== event.data.email_id) throw new Error("Resend returned an unexpected received email.");
    ...
    if (!sender) throw new Error("Received email has no valid sender address.");
